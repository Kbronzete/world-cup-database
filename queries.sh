...
echo "$($PSQL "SELECT name from teams FULL JOIN games ON teams.team_id = games.winner_id
OR teams.team_id = games.opponent_id WHERE year = 2014 AND round = 'Eighth-Final' ORDER BY name")"

echo -e "\nList of unique winning team names in the whole data set:"
echo "$($PSQL "SELECT name FROM teams RIGHT JOIN games ON teams.team_id = games.winner_id GROUP BY name ORDER BY name")"
